...
defmodule Preprocessor do
  ...
  defp anomaly_score_map(forest, x, batch_size) do
    ...
    |> Enum.map(fn
      %{data: data, depth: depth} -> depth + old_c_fun(length(data))
    end)
    |> Service.Novelty.anomaly_score(batch_size, &old_c_fun/1)
...
def experiment(
    ...
    File.write!("csv/depthxperiment/depth_experiment_old#{dataset_name}.csv",
      r1
      |> IO.inspect

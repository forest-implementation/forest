...
def experiment(
    ...
    roc =
      ...
      |> Enum.map(fn {_thresh, _fb, tpr, fpr} -> {tpr, fpr} end)
      ...
      {Enum.at(fbeta,5000), Enum.at(fbeta,6000)}|> IO.inspect
      fbeta |> Enum.max(fn {_,fb1},{_,fb2} -> fb1 >= fb2 end) |> IO.inspect
  ...
  |> Enum.join("\n")
    ...
    File.write!("csv/pokus_nove_body/roc_data#{dataset_name}_#{auc_value}.csv",
      csv_content
    )

...
def experiment(
        ...
        robustfun,
        _anomaly_treshold \\ nil,          # prahy bereme z reálných skóre (sweep)
        tree_count \\ 100,
    ...
    # výběr dimenzí (degenerované cut)
    init_range =
    ...
    # normalizace směru skóre:
    # chceme, aby NIŽŠÍ = více anomální (kompatibilita s Pythonem a pravidlem score<th)
    median = fn xs ->
      ys = Enum.sort(xs)
      len = length(ys)
      if rem(len, 2) == 0 do
        (Enum.at(ys, div(len, 2) - 1) + Enum.at(ys, div(len, 2))) / 2
      else
        Enum.at(ys, div(len, 2))
      end
    end

    r_med = median.(r_scores)
    n_med = median.(n_scores)

    {r_scores, n_scores} =
      if n_med > r_med do
        {Enum.map(r_scores, &(-&1)), Enum.map(n_scores, &(-&1))}
      else
        {r_scores, n_scores}
      end
    ...
    # --- monotónní sweep (pozitivní = novelty, pravidlo score < threshold) ---
    combined =
      Enum.map(r_scores, &{&1, :r}) ++ Enum.map(n_scores, &{&1, :n})
      |> Enum.sort_by(fn {s, _} -> s end, :asc)

    groups = Enum.chunk_by(combined, fn {s, _} -> s end)
    ...
    # doplň sentinely a odstraň duplicitní sousedy
    roc_pts =
    ...
    # --- výstupy do CSV ---
    File.mkdir_p!("conference/csv_our/roc_curves")
    File.mkdir_p!("conference/csv_our/auc")
    File.mkdir_p!("conference/csv_our/confusion")

    # ROC by FPR (na grafy) + alias, a pro kontrolu i by_threshold
    roc_by_fpr = roc_pts |> Enum.sort_by(fn {_th, fpr, _tpr} -> fpr end)
    File.write!("conference/csv_our/roc_curves/#{dataset_name}.csv",
      (["threshold,fpr,tpr"] ++ Enum.map(roc_by_fpr, fn {th, fpr, tpr} -> "#{th},#{fpr},#{tpr}" end))
      |> Enum.join("\n")
    )

    roc_by_th = roc_pts |> Enum.sort_by(fn {th, _fpr, _tpr} -> th end)
    File.write!("conference/csv_our/roc_curves/#{dataset_name}_by_threshold.csv",
      (["threshold,fpr,tpr"] ++ Enum.map(roc_by_th, fn {th, fpr, tpr} -> "#{th},#{fpr},#{tpr}" end))
      |> Enum.join("\n")
    )

    File.write!("conference/csv_our/auc/#{dataset_name}.csv", "auc\n#{auc_value}\n")

    # confusion per skok (blok shodného skóre)
    conf_rows =
    ...
    File.write!("conference/csv_our/confusion/#{dataset_name}.csv",

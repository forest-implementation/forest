# MIX_ENV=example mix run example/adbench_adam_experiment.ex
...
defmodule Preprocessor do
  ...
  # TODO: TADY MENIT CFUN
  # udelej toto pro vsechny regulary co mame
  # zamen jen cfun na jejich cfun
  # a v pythonu udelej nejaky statisticky prehled, mean avg atp
...
def experiment(
        {train, rtest, _ntest, dataset_name},
        robustfun,
    ...
    # Uložit do CSV souboru csv/depth_experiment.csv
    File.write!("csv/nove_hacko_30_depth/stare#{dataset_name}.csv",
      r1
      |> IO.inspect
        |> Enum.map(fn {point, {avg, hfun}} -> "#{point |> :erlang.term_to_binary |> then(fn x-> :crypto.hash(:sha256,x) end) |> Base.encode16},#{avg}, #{hfun}" end)
      |> Enum.join("\n")
    )

    IO.puts("Výstup uložen do csv/depth_experiment.csv")
    {r1, []}
end
...
datasets =
  ...
  # skipuju velke datasety at je to rychlejsi
  |> Enum.reject(fn dataset -> Enum.member?(["3_backdoor", "9_census","11_donors"], dataset) end)
  |> IO.inspect()

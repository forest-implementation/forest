...
# ============================================================
#  LaTeX/PGFPlots styling with fallback (same as your runtime plot)
# ============================================================
try:
    mpl.rcParams.update({
        "text.usetex": True,
...
def read_roc_curve(roc_dir: Path, dataset_file: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Expected columns: threshold,fpr,tpr (only fpr/tpr are used)
    """
...
def add_right_center_legend(ax: plt.Axes) -> None:
    ax.legend(
        loc="center left",
...
def save_plot(path: Path, xlabel: str, ylabel: str, *, fig: plt.Figure, ax: plt.Axes) -> None:
    # No title (to match your runtime styling)
...
def main():
    ...
    for ds_file in common_datasets:
        ...
        for alg_dir in alg_dirs:
            ...
            try:
                ...
                ax.plot(fpr, tpr, label=rf"{alg} (AUC={roc_auc:.4f})")
            except Exception as e:
        ...
        save_plot(
            ...
            xlabel=r"False Positive Rate",
            ylabel=r"True Positive Rate",
            fig=fig,
            ax=ax,
        )
        ...
        for alg_dir in alg_dirs:
            ...
            try:
                rec, prec, pr_auc = read_pr_curve_from_confusion(conf_path)
                ax.plot(rec, prec, label=rf"{alg} (PR AUC$\approx${pr_auc:.4f})")
            except Exception as e:
    ...
    for alg in alg_names:
        ...
        ax.plot(fpr_grid, mean_tpr, label=rf"{alg} (mean AUC={auc_mean:.4f})")
        ax.fill_between(
            fpr_grid,
    ...
    for alg in alg_names:
        ...
        ax.plot(rec_grid, mean_prec, label=rf"{alg} (mean PR AUC$\approx${pr_mean:.4f})")
        ax.fill_between(
            rec_grid,

defmodule Service.Novelty do
  ...
  def split_data(%{:data => [], :ranges => ranges, :depth => d}),
    do: {%{depth: d, ranges: ranges, data: nil}}
  def split_data(%{data: data, ranges: ranges, depth: depth}) do
    ...
    %{true => left_data, false => right_data} =
      data |> Enum.group_by(&decision(&1, {dimension, sp})) |> Map.merge(%{ :false => [], :true => []}, fn _,v1,_ -> v1 end )
